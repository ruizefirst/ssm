...
function PutCookie($key,$value,$cfg_keeptime){
  ...
  setcookie($key,$value.$cfg_cookie_encode,time()+2*3600);
    else
  setcookie($key,$value.$cfg_cookie_encode,time()+$cfg_keeptime*3600);
}
//使Cookie失效
...
function GetCookie($key){
	@session_start();
	 ...
	 //return str_replace($cfg_cookie_encode,'',$_COOKIE[$key]);
}
//获得验证码的值
