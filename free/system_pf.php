...
<title><?php echo $cfg_softname;?>帐款管理</title>
</head>
<body>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
      ...
      <td>
	   ...
	   <form action="system_yf.php?action=seek" name="form1" method="post">
	    <tr>
		 ...
		 $query1="insert into #@__accounts(atype,amoney,abank,dtime,apeople,atext) values('收入','$money','".$row['id']."','$logindate','$username','取消已付账款，单号：$dh')";
		 $sql->executenonequery($query);
		 $sql->executenonequery($query1);
          WriteNote('单号:'.$dh.'的付款已取消.',$logindate,$loginip,$username);
		  $sql->close();
		  showMsg('取消付款操作成功!','system_pf.php');
		 }
     ...
     <tr>
      <td bgcolor="#FFFFFF">
	   ...
	   $moneystring="<b>已付帐款：".$inumber." 项，共计金额：￥".number_format($imoney,2,'.',',')."元</b>";
...
$dlist->SetSource($query);
	   ...
	   <td><center><a href='system_pf.php?action=sure&dh=".$row['r_dh']."&money=$cmoney' title='确定此已付款'>取消付款</a></td>\r\n
	   </tr>";
	   }
