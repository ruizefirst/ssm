<HTML>
<HEAD>
<TITLE>web进销存-Viooma进销存软件</TITLE>
...
if ($action=='login')
{
 if (GetCkVdValue()==$code)
  ...
  $sql="select * from #@__boss where boss='$username' and password='".md5($password)."'";
  $lsql->SetQuery($sql);
  $lsql->Execute();
  ...
  {//可以正常登陆，写登陆数据
  $message="正常登入进销存系统！";
  $_SESSION['VioomaUserID']=$username.$cfg_cookie_encode;
  $_SESSION['rank']=$row['rank'];
  PutCookie('VioomaUserID',$username,$cfg_keeptime);
  PutCookie('rank',$row['rank'],$cfg_keeptime);
  WriteNote($message,$logindate,$loginip,$username);
  $loginsql="update #@__boss set logindate='$logindate',loginip='$loginip' where boss='$username'";
...
<TR>
                ...
                <TD width=39>&nbsp;</TD></TR>
              <TR align=middle>
                <TD colSpan=3 style="text-align:left;line-height:150%;padding:10px;">
            CopyRights &copy;2008~2009 Powered By <a href="http://www.viooma.com" target="_blank">www.ViooMA.com</a> Web进销存 2008版<br>
			联系QQ:475994986&nbsp;&nbsp;技术论坛:<a href="http://bbs.viooma.com" target="_blank">http://bbs.viooma.com/</a>
