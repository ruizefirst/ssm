...
<title><?php echo $cfg_softname;?>已收帐款管理</title>
</head>
<body>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
      ...
      <td>
	   ...
	   <form action="system_ps.php?action=seek" name="form1" method="post">
	    <tr>
		 ...
		 $query1="insert into #@__accounts(atype,amoney,abank,dtime,apeople,atext) values('支出','$money','".$row['id']."','$logindate','$username','取消已收账款，单号：$dh')";
		 $sql->executenonequery($query);
		 $sql->executenonequery($query1);
          WriteNote('成功取消收款单号:'.$dh.'的款项.',$logindate,$loginip,$username);
		  $sql->close();
		  showMsg('取消收款操作成功!','system_ps.php');
		 }
     ...
     <tr>
      <td bgcolor="#FFFFFF">
	   ...
	   $asql->close();
	   $moneystring="<b>已收帐款：".$inumber." 项，共计金额：￥".number_format($imoney,2,'.',',')."元</b>";
...
$dlist->SetSource($query);
	   ...
	   <td><center><a href='system_ps.php?action=sure&dh=".$row['r_dh']."&money=$cmoney' title='确定取消此款已收'>取消收款</a></td>\r\n
	   </tr>";
	   }
