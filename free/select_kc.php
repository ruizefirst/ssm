...
window.opener.document.<?php echo $form ?>.showinfo.value="产品名称："+value+"  规格："+gg+" 库存:"+number; 
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="2">
     ...
     <tr>
      <td bgcolor="#FFFFFF">
	   ...
	   switch($sort){
	    case "1"://按货号
		$query="select * from #@__mainkc,#@__basic where #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_number='$stext' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down'";
		break;
		case "2"://按条码
		$query="select * from #@__mainkc,#@__basic where #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_tm='$stext' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down'";
		break;
		case "3"://按名称
		$query="select * from #@__mainkc,#@__basic where #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_name LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down'";
		break;
		case "4"://按肋记词
		$query="select * from #@__mainkc,#@__basic where #@__mainkc.p_id=#@__basic.cp_number and #@__basic.cp_helpword LIKE '%".$stext."%' and #@__basic.cp_categories='$cp_categories' and #@__basic.cp_categories_down='$cp_categories_down'";
		break;
		}
	   }
	   else
...
$dlist->SetSource($query);
	   ...
	   <td><center><input type='checkbox' name='sel_pro".$row['id']."' value='".$row['cp_name']."' onclick=\"selectpro(this.value,".$row['cp_number'].",'".$row['cp_gg']."','".get_name($row['cp_dwname'],'dw')."','".$row['number']."','".$row['l_id']."','".get_name($row['l_id'],'lab')."')\"></td>\r\n
	   </tr>";
	   }
